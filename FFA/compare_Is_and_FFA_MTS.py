...
#plot the FFA data
FFA_virus = np.load(r'C:\Research_Assistant\work\RA_COVID_immunity\regression_files\FFA_MTS_V_measured_NON_DET_eq_zero_sick_only.npy')
print('FFA_virus',FFA_virus)
log_FFA_virus = np.log10(FFA_virus)
FFA_effective_day = np.load(r'C:\Research_Assistant\work\RA_COVID_immunity\regression_files\FFA_MTS_t_measured_NON_DET_eq_zero_sick_only.npy')
plt.figure()
...
params.add('alpha', value=6.01*(10**(-7)), min=5*(10**(-8)), max=7*(10**(-4)))   #rate that viral particles infect susceptible cells
params.add('beta', value=0.000001, min=0, max=0.0001)    #Clearance rate of infected cells
params.add('gamma', value=100, min=0, max=500)        #Infected cells release virus at rate gamma
...
plt.ylabel('log(FFA)')
plt.title('MTS scatterplot of log(FFA) and log(Is)')
...
plt.ylabel('FFA')
plt.title('MTS scatterplot of FFA and Is')
