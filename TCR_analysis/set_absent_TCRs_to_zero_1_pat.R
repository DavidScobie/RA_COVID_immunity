...
#lam_vals <- c(0,2,0,0,16,8) #dummy array of first timepoint values

#the timepoint 7 dataset
end_time_vals <- subset_pat_439679_alpha_prod_wide$duplicate_count.7
#end_time_vals <- c(0,0,52,6,0,4) #dummy array
...
#print(paste("count_1_thresh",count_1_thresh))
...
# ar_before_1 <- vector()
# count <- 0
# fake_lam_vals <- c(7,2,14,0,5,2,8)
# for (z in 1:max(fake_lam_vals)) {
#   ar_before_1 <- sapply(z,function(x) sum_poisson(x,start=0,stop=1))
#   print(paste("ar_before_1",ar_before_1))
#   if (ar_before_1 < (p_value/2)) {
#      count=count+1 
...
#   }
# }
#  count_1_thresh <- max(fake_lam_vals) - count
#  print(paste("count_1_thresh",count_1_thresh))
#  
#now, go through all the duplicate_count=0 for day 0 and day 7. And make sig=0 if below threshold 
