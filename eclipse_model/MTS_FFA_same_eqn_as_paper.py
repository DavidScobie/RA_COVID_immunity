...
def f(y, t, paras):
    ...
    #the parameters alpha, beta, gamma, delta and mu
    try:
        ...
        alpha, beta, gamma, delta, mu = paras
...
def residual(paras, t, data):
    ...
    x0 = paras['U0'].value, paras['I10'].value, paras['I20'].value, paras['V0'].value
    model = g(t, x0, paras)
...
U0 = 4*(10**(7))  #the number of cells in an adult is 4x10^8
...
params.add('alpha', value=5*(10**(-5)), min=4.9*(10**(-5)), max=5.1*(10**(-5)))   #rate that viral particles infect susceptible cells
params.add('beta', value=4, min=3.9, max=4.1)    #Clearance rate of infected cells
params.add('gamma', value=0.046, min=0.045, max=0.047)        #Infected cells release virus at rate gamma
params.add('delta', value=5, min=4.9, max=5.1)     #clearance rate of virus particles
params.add('mu', value=5, min=4.9, max=5.1)
