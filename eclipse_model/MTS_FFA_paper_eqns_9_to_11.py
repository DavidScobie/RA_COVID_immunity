...
#make the absolute virus data (not the log scale)
...
#multiply all FFA points by 1000 to get the data to be modelled a bit easier
act_div_vir_list_sum_f_e_chop = 1000*act_div_vir_list_sum_f_e_chop
...
params.add('alpha', value=1.2*(10**(-5)), min=0.5*(10**(-5)), max=1.9*(10**(-5)))   #rate that viral particles infect susceptible cells
params.add('beta', value=4, min=2, max=6)    #Clearance rate of infected cells
params.add('gamma', value=0.046, min=0, max=1)        #Infected cells release virus at rate gamma
params.add('delta', value=1.2, min=0, max=3)     #clearance rate of virus particles
params.add('mu', value=1.2, min=0, max=3)
params.add('s', value=1, min=0.99, max=1.01)
params.add('epsilon', value=1, min=0.9, max=1.1)
params.add('tau', value=4, min=3.99, max=4.01)
